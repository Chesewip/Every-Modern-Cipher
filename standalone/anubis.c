...
static void be32_store(u8 *p, u32 v)
{
	p[0] = (u8)(v >> 24);
	p[1] = (u8)(v >> 16);
	p[2] = (u8)(v >>  8);
...
int anubis_setkey(struct anubis_ctx *ctx, const u8 *in_key,
			 ...
			 u8 *dst, const u8 *src, const int R)
{
	 ...
	 */
	for (i = 0; i < 4; i++)
		state[i] = be32_load(&src[4 * i]) ^ roundKey[0][i];
	...
	for (i = 0; i < 4; i++)
		be32_store(&dst[4 * i], inter[i]);
}

void anubis_encrypt(struct anubis_ctx *ctx, u8 *dst, const u8 *src)
{
	anubis_crypt(ctx->E, dst, src, ctx->R);
}

void anubis_decrypt(struct anubis_ctx *ctx, u8 *dst, const u8 *src)
{
